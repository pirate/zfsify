...
`rclone-root-full.cast` contains the actual PTY output, including staging and reconnects. Explicit recording notes identify the SSH boundaries. `rclone-root.cast` and `rclone-root.gif` are selected verbatim terminal output with wrapped lines, an accelerated countdown, and omitted package chatter/idle waits. Recording annotations and reconnect cards are omitted. `rclone-root-selection.json` identifies the excerpts and source times where available. No successful commands, progress values, speeds, or checksums are invented.
