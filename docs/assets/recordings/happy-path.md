...
- [GIF preview](happy-path.gif).
- [Edited asciicast](happy-path.cast): the same selected terminal output.
...
The GIF and edited cast accelerate the genuine 15-second countdown and select actual installer output. Package-manager chatter, long waits, and reboot downtime are shortened or omitted. Only captured terminal output is shown; recording annotations and reconnect cards are omitted. Byte counts, throughput, IOPS, device names, and command results come from the actual run. Lines are wrapped for readability. The full cast retains the captured output and original timestamps; its idle-time hint allows a player to shorten inactivity.
