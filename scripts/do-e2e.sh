...
chmod 700 "$STATE"
IP=
SSH=(ssh -F /dev/null -i "$STATE/key" -o StrictHostKeyChecking=accept-new -o UserKnownHostsFile="$STATE/known_hosts" -o ConnectTimeout=10)
...
"${SSH[@]}" "root@$IP" 'systemd-run --unit=zfs-on-boot-stage --collect /bin/bash -o pipefail -c "curl -fsSL http://127.0.0.1:8765/install.sh | sh"'
ready=0
